...
function validateOptions<TVariables, TData>(options: WatchQueryOptions<TVariables, TData>) {
  const { query, fetchPolicy = DEFAULT_FETCH_POLICY, returnPartialData } = options;
...
function useWatchQueryOptions<TData, TVariables>({
  ...
  options,
  client,
}: UseWatchQueryOptionsHookOptions<TData, TVariables>): WatchQueryOptions<TVariables, TData> {
  ...
  const watchQueryOptions = useDeepMemo<WatchQueryOptions<TVariables, TData>>(() => {
    ...
    return {
      //...watchQueryOptions,
      query,
      errorPolicy: errorPolicy || defaultOptions?.errorPolicy || DEFAULT_ERROR_POLICY,
      fetchPolicy: fetchPolicy || defaultOptions?.fetchPolicy || DEFAULT_FETCH_POLICY,
      ...
      //
      // ここをtureにしないと手動refetchまで、client.resetStore()等の操作を受け付けなくなる
      fetchOnFirstSubscribe: true, //false,
...
function useObservableQueryResult<TData, TVariables>(
